...
|1| Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам?|Нет|MFA не реализована для доступа к внутренним системам, включая CRM и базы данных|
|2| Настроены ли требования к сложности паролей и их регулярное обновление?|Нет|Изменения в команде ИТ-продуктов не отслеживаются специалистом по ИБ|
|3| Проводится ли аудит и пересмотр учётных записей с повышенными привилегиями на регулярной основе?|Частично|Доступы часто зависят от бизнес-логики и не всегда соответствуют минимально необходимым|
|4| Реализован ли принцип минимальных привилегий при назначении ролей и правдоступа?|Частично|Доступы часто зависят от бизнес-логики и не всегда соответствуют минимально необходимым|
|5| Автоматически ли удаляются или деактивируются учётные записи после увольнения или изменения должности сотрудников?|Да|Это важно особенно если сотрудник был уволен с проблемами|
|6| Ведётся ли журналирование всех операций доступа к системам и данным? Кто имеет доступ к журналам?|Частично|Журналы доступны только для некоторых сервисов. Нет единой системы аудита|
...
|1| Разработана ли политика классификации данных, и актуализируется ли она в соответствии с действующим законодательством?|Частично|Анализ данных произведён, но политика формально не закреплена|
|2| Используется ли шифрование на уровне базы данных, файловой системы и при передаче данных по сети?|Частично|Шифрование включено при передаче данных, но не реализовано в БД|
|3| Реализованы ли механизмы управления ключами шифрования?|       |              |
|4| Проводится ли регулярное резервное копирование данных? Хранятся ли резервные копии в защищённых хранилищах?|Да|Резервное копирование выполняется в соответствии с требованиями|
...
|7| Ведётся ли учёт персональных данных, и применяются ли специальные процедуры обработки и хранения для различных типов данных?|Частично|Персональные данные собираются, но нет централизованного контроля обработки|
...
|9| Разработаны ли процессы и системы Data Governance для управления данными?|Нет|Отсутствует единый подход к управлению потоками данных|
...
|13| Применяются ли средства или процессы анонимизации данных?|Нет|Не реализовано, несмотря на работу с персональными и поведенческими данными|
|14| Разработана ли инструкция по процессу восстановления данных?|Нет|              |
| **III. Защита инфраструктуры**|       |              |
|1| Используется ли межсетевой экран для ограничения входящего и исходящего трафика?|Да|Firewall настроен для доступа клиентов и собственников|
|2| Реализована ли сегментация сети на логические зоны (DMZ, корпоративная сеть, гостевая сеть) для ограничения доступа к критическим ресурсам?|Нет|Нет выделенных зон для критичных ресурсов и DMZ|
...
|5| Проводится ли регулярное сканирование уязвимостей операционных систем, приложений и сетевых устройств?                                   |Нет|Процесс отсутствует либо не документирован|
